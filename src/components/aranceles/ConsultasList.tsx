...
const ConsultasList: React.FC<ConsultasListProps> = ({ onBack }) => {
  const [searchTerm, setSearchTerm] = useState('');
  const [currentPage, setCurrentPage] = useState(1);
  const [consultas, setConsultas] = useState<Arancel[]>([]);
  ...
  const loadConsultas = async (page: number = 1, search: string = '') => {
    try {
      setLoading(true);
      setError(null);
  ...
  useEffect(() => {
    loadConsultas(currentPage, searchTerm);
  }, [currentPage]);
      ...
      <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-cyan-50">
        <div className="container mx-auto px-6 py-8">
          <div className="flex items-center mb-8">
            ...
            <div>
              <h1 className="text-3xl font-bold text-gray-800">Aranceles de Consultas</h1>
              <p className="text-gray-600 mt-1">Precios de consultas médicas por especialidad</p>
            </div>
          </div>
          
          <div className="flex items-center justify-center py-12">
            <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-blue-600"></div>
            <span className="ml-3 text-blue-600">Cargando consultas...</span>
          </div>
        </div>
      </div>
    );
      ...
      <div className="container mx-auto px-6 py-8">
        ...
        <div className="bg-white rounded-xl shadow-lg p-6 mb-6">
          <div className="relative max-w-md">
            ...
            <input
              type="text"
              placeholder="Buscar por descripción o especialidad..."
              value={searchTerm}
        ...
        {error && (
          <div className="bg-red-50 border border-red-200 rounded-xl p-4 mb-6">
            ...
            <button
              onClick={() => loadConsultas(currentPage, searchTerm)}
        ...
        <div className="bg-white rounded-xl shadow-lg overflow-hidden">
            ...
            <table className="min-w-full divide-y divide-gray-200">
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {consultas.map((consulta) => (
                  <tr key={consulta.id} className="hover:bg-gray-50 transition-colors duration-200">
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="flex items-center">
                        ...
                        <div>
                          <div className="text-sm font-medium text-gray-900">{consulta.descripcion}</div>
                        </div>
                      </div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
          ...
          <div className="lg:hidden">
            {consultas.map((consulta) => (
              <div key={consulta.id} className="border-b border-gray-200 p-6 last:border-b-0">
                <div className="flex items-center justify-between mb-4">
                  <div className="flex items-center">
                    ...
                    <div>
                      <h3 className="text-lg font-semibold text-gray-900">{consulta.descripcion}</h3>
                    </div>
                  </div>
                  <div className="flex items-center">
          ...
          {consultas.length === 0 && !loading && (
            <div className="text-center py-12">
              ...
              <p className="text-lg text-gray-500">No se encontraron consultas</p>
              {searchTerm && (
